...
""" % (config_indexurl,config_rootdir,config_startpage,config_lang,config_upload,config_data_lang,config_dateinput,config_dateoutput)
    ...
    def saveUser(self):
        ...
        if ('util_user_pwd' in self.form and self.form['util_user_pwd'].value != ''): #Empty fields are discarded automatically
          #Hide user password
          import md5
          user_pwd = md5.new(str(self.form['util_user_pwd'].value)).hexdigest()             
    ...
    def loadGroupTab(self,data):
        ...
        for role in list_roles:
           ...
           jsparam = ''
           if role['name'].encode('utf8') == 'Administrator': #Prevent Administrator group to be edited
    ...
    def loadDBTab(self,data):
        ...
        for onedb in list_dbs:
          ...
          if (self.session.data['dbms'] == onedb['dbms_name']
            ...
            jsparam = "onclick=\"edit_base('%s','%s','%s','%s','%s','%s','%s')\"" % (str(onedb['base_id']),str(onedb['dbms_id']),str(onedb['host']),str(onedb['port']),onedb['db_name'],onedb['user'],str(onedb['pwd']))
          line = line % jsparam
          data['dbs_table'] += line
