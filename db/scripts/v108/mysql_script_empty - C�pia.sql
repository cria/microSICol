...
CREATE VIEW lot_strain_stock_data AS 
  select `lsl`.`id_lot` AS `id_lot`,
    `lsl`.`id_strain` AS `id_strain`,
    `lsl`.`id_container_hierarchy` AS `id_container_hierarchy`,
    `lsl`.`row`,
    `lsl`.`col`,
    `lsl`.`quantity`,
    cast((select sum(quantity) from preservation_strain `ps` where
            `ps`.`id_lot` = `lsl`.`id_lot` and `ps`.`id_origin_container_hierarchy` = `lsl`.`id_container_hierarchy` and `ps`.`origin_row` = `lsl`.`row` and `ps`.`origin_col` = `lsl`.`col`) as unsigned integer) AS `used_qt_pres`,
    cast((select sum(quantity) from distribution_origin_location `dol` where
            `dol`.`id_origin_lot` = `lsl`.`id_lot` and `dol`.`id_origin_container_hierarchy` = `lsl`.`id_container_hierarchy` and `dol`.`origin_row` = `lsl`.`row` and `dol`.`origin_col` = `lsl`.`col`) as unsigned integer) AS `used_qt_dist`,
    cast((select sum(quantity) from str_quality_origin_location `qol` where
            `qol`.`id_origin_lot` = `lsl`.`id_lot` and `qol`.`id_origin_container_hierarchy` = `lsl`.`id_container_hierarchy` and `qol`.`origin_row` = `lsl`.`row` and `qol`.`origin_col` = `lsl`.`col`) as unsigned integer) AS `used_qt_qual`
  from
