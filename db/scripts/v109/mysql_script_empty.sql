DROP DATABASE IF EXISTS sicol_v109;
CREATE DATABASE IF NOT EXISTS sicol_v109 CHARACTER SET utf8;
USE sicol_v109;
...
CREATE TABLE lot_strain (
  ...
  id_strain INTEGER UNSIGNED unsigned NOT NULL,
...
CREATE TABLE preservation_strain (
  ...
  decrease_stock enum('y','n') default NULL,
...
CREATE TABLE distribution_origin_location (
  ...
  decrease_stock enum('y','n') NOT NULL default 'y',
...
CREATE VIEW lot_strain_stock_data AS 
  ...
  where 
    ((ps.id_lot = lsl.id_lot) and (ps.id_origin_container_hierarchy = lsl.id_container_hierarchy) and (ps.origin_row = lsl.row) and (ps.origin_col = lsl.col) and (coalesce(ps.decrease_stock,_utf8'y') <> _utf8'n'))) = 0) and ((
  select 
    count(0) AS `count(*)` 
  from 
    (distribution_origin_location dol join distribution d on((dol.id_distribution = d.id_distribution))) 
  where 
    ((dol.id_origin_lot = lsl.id_lot) and (dol.id_origin_container_hierarchy = lsl.id_container_hierarchy) and (dol.origin_row = lsl.row) and (dol.origin_col = lsl.col) and (coalesce(dol.decrease_stock,_latin1'y') <> _latin1'n'))) = 0) and ((
  select 
    count(0) AS `count(*)` 
  from 
    (str_quality_origin_location sqol join str_quality sq on((sqol.id_quality = sq.id_quality))) 
  where 
    ((sqol.id_origin_lot = lsl.id_lot) and (sqol.id_origin_container_hierarchy = lsl.id_container_hierarchy) and (sqol.origin_row = lsl.row) and (sqol.origin_col = lsl.col) and (coalesce(sqol.decrease_stock,_latin1'y') <> _latin1'n'))) = 0));
	
CREATE VIEW lot_strain_stock AS 
  select 
    ls.id_lot AS id_lot,
    ls.id_strain AS id_strain,
    count(lslm.id_container_hierarchy) AS num_ampoules 
  from 
    (lot_strain ls left join lot_strain_stock_data lslm on(((ls.id_lot = lslm.id_lot) and (ls.id_strain = lslm.id_strain)))) 
  group by 
    ls.id_lot,ls.id_strain;
...
CREATE VIEW preservation_strain_locations AS 
  select 
    ...
    ps.decrease_stock AS decrease_stock 
  from 
...
CREATE VIEW used_origin_all AS 
    select 
      ...
      coalesce(distribution_origin_location.decrease_stock,_latin1'n') AS decrease_stock 
    from 
    ...
    select 
      ...
      coalesce(str_quality_origin_location.decrease_stock,_latin1'y') AS decrease_stock 
    from 
    ...
    select 
      ...
      coalesce(preservation_strain_locations.decrease_stock,_utf8'y') AS decrease_stock 
    from 
...
CREATE VIEW used_origin AS 
  ...
  from 
    distribution_origin_location 
  where 
    (distribution_origin_location.decrease_stock = _latin1'y') union 
  select 
    str_quality_origin_location.id_origin_lot AS id_origin_lot,
    str_quality_origin_location.id_origin_container_hierarchy AS id_origin_container_hierarchy,
    str_quality_origin_location.origin_row AS origin_row,
  ...
  from 
    str_quality_origin_location 
  where 
    (str_quality_origin_location.decrease_stock = _latin1'y') union 
  select 
    preservation_strain_locations.id_lot AS id_origin_lot,
    preservation_strain_locations.id_origin_container_hierarchy AS id_origin_container_hierarchy,
    preservation_strain_locations.origin_row AS origin_row,
  ...
  from 
    preservation_strain_locations 
  where 
    (preservation_strain_locations.decrease_stock = _utf8'y');

CREATE VIEW lot_strain_available_locations AS 
  select 
    current.id_strain AS id_strain,
    current.id_lot AS id_lot,
    current.id_container_hierarchy AS id_container_hierarchy,
    current.row AS row,
    current.col AS col,
    coalesce(source.decrease_stock,_utf8'y') AS `coalesce(source.decrease_stock,'y')` 
  from 
    (((preservation_strain_locations current left join preservation_strain_locations source on(((source.id_origin_container_hierarchy = current.id_container_hierarchy) and (source.id_origin_lot = current.id_lot) and (source.origin_row = current.row) and (source.origin_col = current.col) and (coalesce(source.decrease_stock,_utf8'y') <> _utf8'n')))) left join str_quality_origin_location quality_source on(((quality_source.id_origin_container_hierarchy = current.id_container_hierarchy) and (quality_source.origin_row = current.row) and (quality_source.origin_col = current.col) and (quality_source.id_origin_lot = current.id_lot) and (coalesce(quality_source.decrease_stock,_latin1'y') <> _latin1'n')))) left join distribution_origin_location distribution_source on(((distribution_source.id_origin_container_hierarchy = current.id_container_hierarchy) and (distribution_source.origin_row = current.row) and (distribution_source.origin_col = current.col) and (distribution_source.id_origin_lot = current.id_lot) and (coalesce(distribution_source.decrease_stock,_latin1'y') <> _latin1'n')))) 
  where 
    (isnull(source.id_lot) and isnull(quality_source.id_origin_lot) and isnull(distribution_source.id_origin_lot)) 
  order by 
    current.id_strain,current.id_lot,current.id_container_hierarchy,current.row,current.col;
