...
CREATE TABLE sys_data_lang (
  ...
  lang_index INTEGER UNIQUE NOT NULL,
  PRIMARY KEY(id_subcoll, id_coll, data_lang),
  FOREIGN KEY(id_subcoll, id_coll) REFERENCES subcoll(id_subcoll, id_coll)
);
...
CREATE INDEX FK_coll_subcoll1 ON access(id_subcoll, id_coll);
CREATE INDEX FK_coll_subcoll2 ON sys_config(id_subcoll, id_coll);
CREATE INDEX FK_coll_subcoll3 ON sys_data_lang(id_subcoll, id_coll);
